...
OPERATION_REGISTRY: dict[str, OperationSpec] = {
    ...
    "add_calculated_column": OperationSpec(
        ...
        summary="Work out a new column from the others with a formula, such as basic * 0.12. The formula may also choose between two numbers: price * 0.9 if quantity > 100 else price.",
        inputs=(SOURCE_ROLE,),
        params=(
            ParamSpec(
                name="new_column",
            ...
            ParamSpec(
                ...
                help="Use column names, numbers, + - * / and brackets. Put a name in square brackets if it has spaces, like [net sales] * 0.1. You can also pick between two numbers with a test: price * 0.9 if quantity > 100 else price. Tests can use > < >= <= = and !=. The answer is always a number - use 'Add an if/else column' when the answer is text.",
            ),
            ParamSpec(
                name="decimals",
    ...
    "row_percentage_of_total": OperationSpec(
        operation="row_percentage_of_total",
        label="Percentage of total",
        category=CATEGORY_CALCULATE,
        summary="Each row's share of the column's total, as a percentage - this branch's share of all sales.",
        inputs=(SOURCE_ROLE,),
        params=(
            ParamSpec(
                name="column",
                ...
                kind=ParamKind.COLUMN,
                from_role="source",
                help="The column to share out, such as amount. The whole column is totalled once and every row is divided by that total.",
            ),
            ParamSpec(
                name="decimals",
                ...
                max_value=6,
                required=False,
                help="How many decimal places the percentage is rounded to. 2 gives 12.34%.",
            ),
            ParamSpec(
                name="new_column",
                label="New column name",
                kind=ParamKind.TEXT,
                required=False,
                help="Leave empty to name it after the column, such as 'amount % of total'.",
            ),
        ),
        default_output="in_place",
        output_name_hint="{source}_shares",
        apply=ops_derive.apply_row_percentage_of_total,
        describe=ops_derive.describe_row_percentage_of_total,
        validate=ops_derive.validate_row_percentage_of_total,
        required_columns=ops_derive.required_row_percentage_of_total,
    ),
    "absolute_value": OperationSpec(
        operation="absolute_value",
        label="Drop the minus sign",
        category=CATEGORY_CALCULATE,
        summary="Turn negatives into positives - a credit of -500 becomes 500.",
        inputs=(SOURCE_ROLE,),
        params=(
            ParamSpec(
                name="column",
                ...
                kind=ParamKind.COLUMN,
                from_role="source",
                help="The column whose minus signs you want gone. Brackets, as in (500), are read as a negative too.",
            ),
            ParamSpec(
                name="mode",
                label="Write the answer",
                kind=ParamKind.CHOICE,
                choices=tuple(ops_derive.ABSOLUTE_MODES),
                default=ops_derive.ABSOLUTE_MODES[0],
                help="Either the column itself is changed, or the answer goes in a new column beside it.",
            ),
            ParamSpec(
                name="new_column",
                label="New column name",
                ...
                required=False,
                depends_on="mode",
                depends_value="add a new column",
                help="Leave empty to name it after the column, such as 'amount size'.",
            ),
        ),
        default_output="in_place",
        output_name_hint="{source}_positive",
        apply=ops_derive.apply_absolute_value,
        describe=ops_derive.describe_absolute_value,
        validate=ops_derive.validate_absolute_value,
        required_columns=ops_derive.required_absolute_value,
    ),
    "row_min_max": OperationSpec(
        operation="row_min_max",
        label="Smallest or largest across columns",
        category=CATEGORY_CALCULATE,
        summary="Compare columns on the same row - the higher of budget and actual, row by row.",
        inputs=(SOURCE_ROLE,),
        params=(
            ParamSpec(
                name="columns",
                ...
                kind=ParamKind.COLUMNS,
                from_role="source",
                help="Pick two or more number columns, such as budget and actual. This compares across the row, not down the table.",
            ),
            ParamSpec(
                name="which",
                label="Keep the",
                kind=ParamKind.CHOICE,
                choices=tuple(ops_derive.ROW_EXTREMES),
                default=ops_derive.ROW_EXTREMES[0],
                help="Whether each row keeps the biggest of those columns or the smallest.",
            ),
            ParamSpec(
                name="new_column",
                label="New column name",
                kind=ParamKind.TEXT,
                required=False,
                help="Leave empty to name it after the choice, such as 'largest of the columns'.",
            ),
        ),
        default_output="in_place",
        output_name_hint="{source}_compared",
        apply=ops_derive.apply_row_min_max,
        describe=ops_derive.describe_row_min_max,
        validate=ops_derive.validate_row_min_max,
        required_columns=ops_derive.required_row_min_max,
    ),
    # ----------------------------------------------------------------------------------
    # Summarise
    # ----------------------------------------------------------------------------------
    ...
    "combine_columns": OperationSpec(
        operation="combine_columns",
        label="Combine columns",
        category=CATEGORY_COLUMN,
        summary="Join two or more columns into one - first name and surname into a full name.",
        inputs=(SOURCE_ROLE,),
        params=(
            ParamSpec(
                name="columns",
                ...
                kind=ParamKind.COLUMNS,
                from_role="source",
                help="Pick them in the order they should appear, such as Name then Surname. At least two.",
            ),
            ParamSpec(
                name="joiner",
                label="Put between them",
                kind=ParamKind.CHOICE,
                choices=tuple(ops_columns.JOINER_CHOICES),
                default="space",
                help="What goes between the values: space gives 'Ravi Kumar', dash gives 'Ravi-Kumar'. A blank value is left out, so no double or dangling separators.",
            ),
            ParamSpec(
                name="custom_joiner",
                label="Your own separator",
                kind=ParamKind.TEXT,
                placeholder=" | ",
                required=False,
                depends_on="joiner",
                depends_value=ops_columns.OTHER_JOINER,
                help="The exact text to put between the values, such as ' | ' or ' - '. Taken literally, spaces included.",
            ),
            ParamSpec(
                name="new_column",
                label="New column name",
                kind=ParamKind.TEXT,
                required=False,
                help="Leave empty to name it after the columns, such as 'Name Surname'.",
            ),
        ),
        default_output="in_place",
        output_name_hint="{source}_combined",
        apply=ops_columns.apply_combine_columns,
        describe=ops_columns.describe_combine_columns,
        validate=ops_columns.validate_combine_columns,
        required_columns=ops_columns.required_combine_columns,
    ),
    "extract_by_position": OperationSpec(
        operation="extract_by_position",
        label="Take part of a code",
        category=CATEGORY_COLUMN,
        summary="Pull characters out of a code by position - the first 3 of INV000123 give INV.",
        inputs=(SOURCE_ROLE,),
        params=(
            ParamSpec(
                name="column",
                label="Column holding the codes",
                kind=ParamKind.COLUMN,
                from_role="source",
                help="The column to read, such as invoice_no. Use this when there is no separator to split on.",
            ),
            ParamSpec(
                name="start",
                label="Start at character",
                kind=ParamKind.INTEGER,
                default=1,
                min_value=1,
                help="1 is the first character. 4 starts after the first three.",
            ),
            ParamSpec(
                name="length",
                ...
                default=3,
                min_value=1,
                help="How many to take from the start position. Rows that are shorter come out blank.",
            ),
            ParamSpec(
                name="new_column",
                label="New column name",
                kind=ParamKind.TEXT,
                required=False,
                help="Leave empty to name it after the column and the positions, such as 'invoice_no 1-3'.",
            ),
        ),
        default_output="in_place",
        output_name_hint="{source}_extracted",
        apply=ops_columns.apply_extract_by_position,
        describe=ops_columns.describe_extract_by_position,
        validate=ops_columns.validate_extract_by_position,
        required_columns=ops_columns.required_extract_by_position,
    ),
    # ----------------------------------------------------------------------------------
    # Dates
    # ----------------------------------------------------------------------------------
    ...
    "add_today_date": OperationSpec(
        operation="add_today_date",
        label="Today's date",
        category=CATEGORY_DATES,
        summary="Add a column holding today's date, the same in every row - an 'as at' stamp.",
        inputs=(SOURCE_ROLE,),
        params=(
            ParamSpec(
                name="new_column",
                ...
                default="Loaded_On",
                required=False,
                help="What to call the column, such as Loaded_On. Each time the pipeline runs it stamps that day's date.",
            ),
        ),
        default_output="in_place",
        output_name_hint="{source}_stamped",
        apply=ops_derive.apply_add_today_date,
        describe=ops_derive.describe_add_today_date,
        validate=ops_derive.validate_add_today_date,
        required_columns=ops_derive.required_add_today_date,
    ),
    "shift_date": OperationSpec(
        operation="shift_date",
        label="Add or subtract days",
        category=CATEGORY_DATES,
        summary="Move a date column by a number of days - a due date 30 days after the invoice date.",
        inputs=(SOURCE_ROLE,),
        params=(
            ParamSpec(
                name="column",
                ...
                kind=ParamKind.COLUMN,
                from_role="source",
                help="The date to move, such as invoice_date. Dates are read day-first, so 03/04/2025 is the 3rd of April.",
            ),
            ParamSpec(
                name="days",
                label="Days to add",
                kind=ParamKind.INTEGER,
                default=30,
                min_value=-ops_derive.MAX_DAY_SHIFT,
                max_value=ops_derive.MAX_DAY_SHIFT,
                help="30 moves each date 30 days later. Type a minus number, such as -7, to move earlier.",
            ),
            ParamSpec(
                name="new_column",
                label="New column name",
                kind=ParamKind.TEXT,
                required=False,
                help="Leave empty to name it after the column and the shift, such as 'invoice_date plus 30 days'.",
            ),
        ),
        default_output="in_place",
        output_name_hint="{source}_dated",
        apply=ops_derive.apply_shift_date,
        describe=ops_derive.describe_shift_date,
        validate=ops_derive.validate_shift_date,
        required_columns=ops_derive.required_shift_date,
    ),
    "month_edge": OperationSpec(
        operation="month_edge",
        label="Start or end of month",
        category=CATEGORY_DATES,
        summary="Turn any date into the 1st or the last day of its month, so monthly rows line up.",
        inputs=(SOURCE_ROLE,),
        params=(
            ParamSpec(
                name="column",
            ...
            ParamSpec(
                ...
                label="Take the",
                kind=ParamKind.CHOICE,
                choices=tuple(ops_derive.MONTH_EDGES),
                default=ops_derive.MONTH_EDGES[0],
                help="Any date in March 2026 becomes 01/03/2026 or 31/03/2026. February's end knows about leap years.",
            ),
            ParamSpec(
                name="new_column",
                label="New column name",
                kind=ParamKind.TEXT,
                required=False,
                help="Leave empty to name it after the column and the choice.",
            ),
        ),
        default_output="in_place",
        output_name_hint="{source}_dated",
        apply=ops_derive.apply_month_edge,
        describe=ops_derive.describe_month_edge,
        validate=ops_derive.validate_month_edge,
        required_columns=ops_derive.required_month_edge,
    ),
    # ----------------------------------------------------------------------------------
    # Tidy up
    # ----------------------------------------------------------------------------------
